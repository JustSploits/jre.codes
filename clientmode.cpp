...
void Hooks::OverrideView( CViewSetup* view ) {
	...
	if( g_hooks.b[ XOR( "removeScopeOverlay" ) ] ) {
		if( g_cl.m_local && g_cl.m_local->m_bIsScoped( ) )
			...
			game::SetPostProcess( true );
	}

	// remove scope edge blur.
	if( g_cl.m_local && g_hooks.b[ XOR( "disablePostProcessing" ) ] )
		game::SetPostProcess( false );
	else
		game::SetPostProcess( true );
...
bool Hooks::CreateMove( float time, CUserCmd* cmd ) {
	...
	if (ret) {
		...
		g_csgo.m_prediction->SetLocalViewAngles(cmd->m_view_angles);
	}
