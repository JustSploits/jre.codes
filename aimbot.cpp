...
void AimPlayer::UpdateAnimations( LagRecord* record ) {
	...
	if( m_weapon && !m_weapon->IsKnife( ) ) {
		if( record->m_lag > 0 ) {
			const auto& shot_tick = game::TIME_TO_TICKS( m_weapon->m_fLastShotTime( ) );
			...
			if( shot_tick > old_simulation_ticks && simulation_ticks >= shot_tick ) {
				...
				if( abs( simulation_ticks - shot_tick ) > record->m_lag + 2 )
					m_last_nonshot_pitch = record->m_eye_angles.x;
			}
		}
	...
	if( record->m_shot == 1 ) {
		if( record->m_lag > 1 ) {
			if( record->m_eye_angles.x < 70.0f ) {
				if( m_last_nonshot_pitch > 80.0f )
					record->m_eye_angles.x = m_last_nonshot_pitch;
			}
		}
	}
	...
	//if( m_records.size( ) > 2 ) {
	//	bool bChoked = game::TIME_TO_TICKS( m_player->m_flSimulationTime( ) - m_records.front( )->m_sim_time ) > 1;
	//	bool bInAir = false;
	//
	//	if( !( m_player->m_fFlags( ) & FL_ONGROUND ) || !( m_records.front( )->m_flags & FL_ONGROUND ) )
	//		bInAir = true;
	//
	//	if( bChoked ) {
	//		m_player->m_vecOrigin( ) = m_records.front( )->m_origin;
	//		m_player->SetAbsOrigin( m_records.front( )->m_origin );
	//		m_player->m_vecVelocity( ) = m_records.front( )->m_velocity;
	//		m_player->m_fFlags( ) = m_records.front( )->m_flags;
	//	}
	//
	//	vec3_t data_origin = m_player->m_vecOrigin( );
	//	vec3_t data_velocity = m_player->m_vecVelocity( );
	//	int data_flags = m_player->m_fFlags( );
	//
	//	if( bChoked ) {
	//		SimulateMovement( data_velocity, data_origin, m_player, data_flags, bInAir );
	//		//g_lagcomp.PlayerMove( );
	//
	//		m_player->m_vecOrigin( ) = data_origin;
	//		m_player->SetAbsOrigin( data_origin );
	//		m_player->m_vecVelocity( ) = data_velocity;
	//
	//		m_player->m_fFlags( ) &= 0xFFFFFFFE;
	//		auto penultimate_record = *std::prev( m_records.end( ), 2 );
	//		if( ( m_records.front( )->m_flags & FL_ONGROUND ) && ( penultimate_record->m_flags & FL_ONGROUND ) )
	//			m_player->m_fFlags( ) |= 1;
	//
	//		if( *( float* )( ( uintptr_t )m_player->GetAnimOverlay( 0 ) + 0x138 ) > 0.f )
	//			m_player->m_fFlags( ) |= 1;
	//	}
	//}
	...
	record->m_setup = m_player->SetupBones( record->m_bones, 128, 0x100, record->m_anim_time );
...
void AimPlayer::OnNetUpdate( Player* player ) {
	...
	/*if( m_records.size( ) > 0 ) {
		if( m_flCycle == player->m_AnimOverlay( )[ animstate_layer_t::ANIMATION_LAYER_ALIVELOOP ].m_cycle
			&& m_flPlaybackRate == player->m_AnimOverlay( )[ animstate_layer_t::ANIMATION_LAYER_ALIVELOOP ].m_playback_rate ) {

			// fake update.
			player->m_flSimulationTime( ) = player->m_flOldSimulationTime( ); // s/o estk :^))))
			return;
		}
	}*/

	m_flCycle = player->m_AnimOverlay( )[ animstate_layer_t::ANIMATION_LAYER_ALIVELOOP ].m_cycle;
	m_flPlaybackRate = player->m_AnimOverlay( )[ animstate_layer_t::ANIMATION_LAYER_ALIVELOOP ].m_playback_rate;
...
bool Aimbot::CanDT( ) {
	...
	return g_cl.m_local->alive( )
		...
		&& g_hooks.auto_check( XOR( "rapidfire_key" ) )
		&& g_hooks.b[ XOR( "rapidfire" ) ];
}
...
void Aimbot::DoubleTap( ) {
	...
	if( CanDT( ) && !g_csgo.m_gamerules->m_bFreezePeriod( ) ) {
		if( g_hooks.auto_check( XOR( "rapidfire_key" ) ) && g_hooks.b[ XOR( "rapidfire" ) ] ) {
			...
			auto can_shift_shot = CanFireWithExploit( 14 );
...
void AimPlayer::SetupHitboxes( LagRecord* record, bool history ) {
	...
	//if( !( g_hooks.auto_check( "damage1_key" ) && g_hooks.b[ XOR( "body_aim_enable" ) ] ) && hitbox[ 0 ] && ( record->m_mode == Resolver::Modes::RESOLVE_NONE || record->m_mode == Resolver::Modes::RESOLVE_WALK || record->m_mode == Resolver::Modes::RESOLVE_BODY ) )
...
void Aimbot::find( ) {
	...
	for( const auto& t : m_targets ) {
		...
		if( g_hooks.i[ XOR( "lagfix_mide" ) ] > 0 && ( g_lagcomp.StartPrediction( t ) || t->m_records.front( ).get( )->m_invalid || g_csgo.m_cvar->FindVar( HASH( "cl_lagcompensation" ) )->GetInt( ) == 0 ) ) {
			...
			if( g_hooks.i[ XOR( "lagfix_mide" ) ] == 2 )
				continue;
		...
		// history aim is possible at this point.
			...
			if( g_hooks.i[ XOR( "accuracy_boost" ) ] == 4 ) {
				...
				// iterate records.
				for( const auto& it : t->m_records ) {
					if( it->dormant( ) || it->immune( ) || !it->valid( ) )
						continue;

					t->SetupHitboxes( it.get( ), false );
					if( t->m_hitboxes.empty( ) )
						continue;

					// try to select best record as target.
					if( t->GetBestAimPosition( tmp_pos, hitbox, tmp_damage, it.get( ) ) && SelectTarget( it.get( ), tmp_pos, tmp_damage ) ) {
						// if we made it so far, set shit.
						best.player = t->m_player;
						best.pos = tmp_pos;
						best.damage = tmp_damage;
				...
				LagRecord* middle = g_resolver.FindFirstRecord( t );
...
bool Aimbot::CheckHitchance( Player* player, int hitbox, const ang_t& angle ) {
	...
	if( total_hits >= needed_hits ) {
		if( autowalls_hit >= ( ( chance / HITCHANCE_MAX ) * autowalls_done ) )
			return true;
	}
...
bool Aimbot::SelectTarget( LagRecord* record, const vec3_t& aim, float damage ) {
	...
	if( true ) {
		...
		if( math::GetFOV( g_cl.m_view_angles, g_cl.m_shoot_pos, aim ) > g_hooks.f[ XOR( "fov_amount" ) ] )
			return false;
	}
...
void Aimbot::apply( ) {
	...
	if( attack || attack2 ) {
		...
		if( m_target ) {
			...
			float lerp_remainder = fmodf( g_cl.m_lerp, g_csgo.m_globals->m_interval );

			// Get real simulation time and calculate interp fraction
			float real_target_time = m_record->m_sim_time;
			float frac = 0.f;
			if( lerp_remainder > 0.f ) {
				real_target_time += g_csgo.m_globals->m_interval - lerp_remainder;
				// Trigger float inaccuracies (if there is any) and get same frac as the server
				frac = ( real_target_time - m_record->m_sim_time ) / g_csgo.m_globals->m_interval;
			}
		...
		// store fired shot.
		g_shots.OnShotFire( m_target ? m_target : nullptr, m_target ? m_damage : -1.f, g_cl.m_weapon_info->m_bullets, m_target ? m_record : nullptr, m_target ? vec3_t{} : m_aim );
