...
ang_t math::calc_angle(vec3_t src, vec3_t dst)
{
    vec3_t forward = dst - src;
...
vec3_t math::CalcAngle( const vec3_t& vecSource, const vec3_t& vecDestination ) {
    ...
    vAngle.x = float( atanf( float( delta.z / hyp ) ) * M_RADPI );
    vAngle.y = float( atanf( float( delta.y / delta.x ) ) * M_RADPI );
    vAngle.z = 0.0f;
...
float math::simple_spline( float value ) {
    auto value_squared = value * value;
    return 3.0f * value_squared - 2.0f * value_squared * value;
}

float math::simple_spline_remap_val_clamped( float value, float a, float b, float c, float d ) {
    if( a == b ) //-V550
        return value >= b ? d : c;

    auto c_value = std::clamp( ( value - a ) / ( b - a ), 0.0f, 1.0f );
