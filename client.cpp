...
//	std::string text = tfm::format(XOR("jre.rest [release] | %s | delay: %ims | tick: %i | fps: %i | %s"), g_cl.m_user, ms, rate, fps, time.str().data());
...
//
//	// text.
//	render::menu_shade.string(m_width - 14, 14, { 255, 255, 255, 255 }, tfm::format(XOR(" [release] | %s | delay: %ims | tick: %i | fps: %i | %s"), g_cl.m_user, ms, rate, fps, time.str().data()), render::ALIGN_RIGHT);
//	render::menu_shade.string(m_width - 14 - render::menu_shade.size(tfm::format(XOR(" [release] | %s | delay: %ims | tick: %i | fps: %i | %s"), g_cl.m_user, ms, rate, fps, time.str().data())).m_width, 14, col_accent, tfm::format(XOR(".codes")), render::ALIGN_RIGHT);
//	render::menu_shade.string(m_width - 14 - render::menu_shade.size(tfm::format(XOR(".codes [release] | %s | delay: %ims | tick: %i | fps: %i | %s"), g_cl.m_user, ms, rate, fps, time.str().data())).m_width, 14, { 255, 255, 255, 255 }, tfm::format(XOR("jre")), render::ALIGN_RIGHT);
//
//}
void Client::DrawHUD( ) {
	if( !g_csgo.m_engine->IsInGame( ) || !g_hooks.b[ XOR( "watermark" ) ] )
	...
	// get fps.
	int fps = 1.f / g_csgo.m_globals->m_abs_frametime;
	std::string text = tfm::format( XOR( "jre.rest [alpha] | %s | delay: %ims | tick: %i | fps: %i | %s" ), g_cl.m_user, ms, rate, fps, time.str( ).data( ) );
	render::FontSize_t size = render::menu_shade.size( text );
	...
	// text.
	render::menu_shade.string( m_width - 14, 14, { 255, 255, 255, 255 }, tfm::format( XOR( " [alpha] | %s | delay: %ims | tick: %i | fps: %i | %s" ), g_cl.m_user, ms, rate, fps, time.str( ).data( ) ), render::ALIGN_RIGHT );
	render::menu_shade.string( m_width - 14 - render::menu_shade.size( tfm::format( XOR( " [alpha] | %s | delay: %ims | tick: %i | fps: %i | %s" ), g_cl.m_user, ms, rate, fps, time.str( ).data( ) ) ).m_width, 14, col_accent, tfm::format( XOR( ".codes" ) ), render::ALIGN_RIGHT );
	render::menu_shade.string( m_width - 14 - render::menu_shade.size( tfm::format( XOR( ".rest [alpha] | %s | delay: %ims | tick: %i | fps: %i | %s" ), g_cl.m_user, ms, rate, fps, time.str( ).data( ) ) ).m_width, 14, { 255, 255, 255, 255 }, tfm::format( XOR( "jre" ) ), render::ALIGN_RIGHT );
}
...
void Client::print( const std::string text, ... ) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf( colors::green, XOR( "[jre.rest] " ) );
...
void Client::ClanTag( ) {
	...
	auto SetClanTag = [ & ]( std::string tag ) -> void {
		...
		SetClanTagFn( tag.c_str( ), XOR( "java jre codenz" ) );
	};

	std::string szClanTag = XOR( "       java jre codenz       " );
	...
	// gamesense.
		...
		if( iPrevFrame != ( int )( g_csgo.m_globals->m_curtime * 2.7f ) % 18 ) {
			switch( int( g_csgo.m_globals->m_curtime * 2.7f ) % 18 ) {
				...
				case 1: SetClanTag( XOR( "        bl" ) ); break;
				case 2: SetClanTag( XOR( "       blu" ) ); break;
				case 3: SetClanTag( XOR( "      bluh" ) ); break;
				case 4: SetClanTag( XOR( "     bluhg" ) ); break;
				case 5: SetClanTag( XOR( "    bluhga" ) ); break;
				case 6: SetClanTag( XOR( "   bluhgan" ) ); break;
				case 7: SetClanTag( XOR( "  bluhgang" ) ); break;
				case 8: SetClanTag( XOR( " bluhgang" ) ); break;
				case 9: SetClanTag( XOR( "bluhgang " ) ); break;
				case 10:SetClanTag( XOR( "luhgang  " ) ); break;
				case 11:SetClanTag( XOR( "uhgang   " ) ); break;
				case 12:SetClanTag( XOR( "hgang    " ) ); break;
				case 13:SetClanTag( XOR( "gang     " ) ); break;
				case 14:SetClanTag( XOR( "ang      " ) ); break;
				case 15:SetClanTag( XOR( "ng       " ) ); break;
				case 16:SetClanTag( XOR( "g        " ) ); break;
